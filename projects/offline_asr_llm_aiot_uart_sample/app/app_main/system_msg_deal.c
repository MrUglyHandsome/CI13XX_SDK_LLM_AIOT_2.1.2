...
void enter_wakeup_deal_v1(uint32_t exit_wakup_ms, cmd_handle_t cmd_handle )
{
    xSemaphoreTake(WakeupMutex, portMAX_DELAY);
    ...
    #endif
    {
        ...
        #endif
        {
            ...
            {
                #if PLAY_ENTER_WAKEUP_EN && !WMAN_PLAY_EN
                prompt_play_by_cmd_handle_v1(cmd_handle, -1, play_enter_wakeup_done_cb,true);
                #else
                play_enter_wakeup_done_cb(cmd_handle);
                #endif
            }
            ...
            {
                #if PLAY_ENTER_WAKEUP_EN && !WMAN_PLAY_EN
                prompt_play_by_cmd_handle_v1(cmd_handle, -1, default_play_done_callback,true);
                #else
                default_play_done_callback(cmd_handle);
                #endif
            }
